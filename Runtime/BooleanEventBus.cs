using UnityEngine;

namespace Incantium.Events
{
    /// <summary>
    /// Class for the <see cref="bool"/> sending event. Subscribe and invoke <see cref="EventBus{T}.onSend"/> to connect
    /// multiple objects together without a direct dependency.
    /// </summary>
    /// <seealso href="https://github.com/Incantium/Scriptable-Event/blob/main/API~/EventBus.md">EventBus</seealso>
    [CreateAssetMenu(menuName = "Events/Send/Boolean", order = 4)]
    public sealed class BooleanEventBus : EventBus<bool> {}
}